...
% Macroeconomic Outlook & Market Views
\begin{quote}
...
% Understanding of Client (PPT) & Case Problem (CBA x UNIT Case Competition 2025 Case Brief.pdf)
\begin{quote}
...
% Risk Identification & Analysis for PPT
\begin{quote}
...
% Proposed Hedging Strategy (Interest Rate Caps)
\begin{quote}
...
% The \$19.08 million cash injection for 2026 in the Bear case (+25bp scenario), is calculated as the annualized sum required for Prime Property Trust (PPT) to meet its 1.75x Interest Coverage Ratio (ICR) covenant. This calculation would involve taking PPT's forecasted FY26 EBITDA (given as \$8.003 million for FY26 in the appendix ), determining the total annual interest expense under the "+25bp" scenario considering the effect of the 4.00\% interest rate cap, and then calculating the cash injection (CI) needed via a formula such as CI = (1.75 * Total Annual Interest Expense) - EBITDA. For instance, if the FY26 base BBSY is 3.49\%, a +25bp increase results in a BBSY of 3.74\%; since this is below the 4.00\% cap strike, the cap would not pay out, and the interest expense would be approximately \$15.42 million (\$100m \times (3.74\%+1.20\%) + \$200m \times (3.74\%+1.50\%)), leading to a CI of roughly (1.75×\$15.42M)−\$8.003M≈\$18.98M, with the precise \$19.08M figure potentially resulting from specific model rounding or slightly different base inputs in the presentation. Importantly, the cap does fully protect against further rate rises beyond this +25bp scenario in terms of additional cash injections needed for that year due to BBSY increases; the note on Slide 10 explicitly states that for this Bear case, "any further interest rate hikes will not change these figures". This is because if BBSY were to rise above the cap's 4.00\% strike rate, PPT's effective benchmark interest rate for its debt would be capped at 4.00\%, thereby fixing the maximum interest expense related to BBSY fluctuations and consequently fixing the maximum cash injection required to maintain the 1.75x ICR for any higher BBSY levels.
